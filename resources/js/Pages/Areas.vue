...
const todos_los_enlaces = toRefs(props).todos_los_enlaces;
...
let enlaces_auto = [];
todos_los_enlaces.value.forEach((area) => {
    enlaces_auto.push({
        value: area.id,
        name: area.name,
    });
...
const nuevoAreaDo = () => {
    ...
    form.post(route("areas.store"), {
            ...
            notify(
                {
                    group: "main",
                    title: "Area creada",
                    text: "El Área se ha creado exitosamente",
                },
                3000
            );
        ...
        onError: (errors) => {
            ...
            error.enlace = errors.enlace;
            error.enlace_cargo = errors.enlace_cargo;
            error.titular = errors.titular;
            error.titular_cargo = errors.titular_cargo;
        },
    });
};
...
const editaArea = (area) => {
    ...
    if (area.enlace) {
        form.enlace.value = area.enlace.id;
        form.enlace.name = area.enlace.name;
    } else {
        form.enlace.value = "";
        form.enlace.name = "";
    }
    form.enlace_cargo = area.enlace_cargo;
    if (area.titular) {
        form.titular.value = area.titular.id;
        form.titular.name = area.titular.name;
    } else {
        form.titular.value = "";
        form.titular.name = "";
    }
    form.titular_cargo = area.titular_cargo;
    error.nombre = "";
    error.descripcion = "";
    error.area_padre = "";
    error.enlace = "";
...
<template>
    <AppLayout title="Areas">
                ...
                >
                    ...
                    <Tabla v-if="areas.data.length > 0">
                        <template #col>
                            ...
                            <th class="px-4 py-1 w-52">Titular</th>
                            <th class="px-4 py-1 w-52">Enlace</th>
                            <th class="px-4 py-1 w-64">
                                <font-awesome-icon
                                    icon="angle-up"
        ...
        <form
            class="flex flex-col"
            @submit.prevent="accion === 'new' ? nuevoAreaDo : editaAreaDo"
        >
            <JetDialogModal
                v-if="estadoModalArea === true"
                :show="estadoModalArea"
                @close="estadoModalArea = false"
                max-width="md"
                id="elmodal"
            >
                <template #title
                    >{{ accion === "new" ? "Crear nuevo área" : "Editar área" }}
                    <JetDangerButton
                        v-if="accion === 'edit'"
                        @click="estadoModalEliminaArea = true"
                ...
                <template #content>
                    <label class="block mb-1">
                        ...
                        <JetInput
                            id="nombre"
                            v-model="form.nombre"
                            placeholder="Nombre del área o unidad administrativa"
                            type="text"
                    ...
                    <label v-if="accion === 'edit'" class="block mb-1">
                        ...
                        <Autocomplete
                            ...
                            class="mt-1 block w-full"
                            :options="enlaces_auto"
                        />
                        <JetInputError :message="error.titular" class="mt-2" />
                    </label>
                    <label v-if="accion === 'edit'" class="block mb-1">
                        <JetLabel
                            for="titular_cargo"
                        ...
                        <JetInput
                            id="titular_cargo"
                            v-model="form.titular_cargo"
                            placeholder="Cargo del titular"
                            type="text"
                            class="mt-1 block w-full"
                        />
                        <JetInputError
                            :message="error.titular_cargo"
                            class="mt-2"
                        />
                    </label>
                    <label v-if="accion === 'edit'" class="block mb-1">
                        ...
                        <Autocomplete
                            ...
                            :options="enlaces_auto"
                        />
                        <JetInputError :message="error.enlace" class="mt-2" />
                    </label>
                    <label v-if="accion === 'edit'" class="block mb-1">
                        <JetLabel
                            for="enlace_cargo"
                        ...
                        <JetInput
                            id="enlace_cargo"
                            v-model="form.enlace_cargo"
                            placeholder="Cargo del enlace"
                            type="text"
                            class="mt-1 block w-full"
                        />
                        <JetInputError
                            :message="error.enlace_cargo"
                            class="mt-2"
                        />
                    </label>
                    <label v-if="accion === 'edit'" class="block mb-1">
                        ...
                        <Autocomplete
                            id="area_padre"
                            v-model="form.area_padre"
                            placeholder="Escriba una opción..."
                            class="mt-1 block w-full"
                            :options="areas_auto"
                        />
                        <JetInputError
                            :message="error.area_padre"
                            class="mt-2"
                        />
                    </label>
                </template>
        ...
        <JetDialogModal
            v-if="estadoModalEliminaArea === true"
            :show="estadoModalEliminaArea"
            @close="estadoModalEliminaArea = false"
            max-width="md"
        >
            <template #title>Eliminar area</template>
            <template #content>
                <h4 class="block mb-1 text-lg">
                    ¿Realmente desea eliminar esta área?
                </h4>
            ...
            <template #footer>
                <div class="w-1/2">
                    <JetButton
                        @click="estadoModalEliminaArea = false"
                        class="bg-opacity-95"
                        type="cancel"
                        :disabled="form.processing"
                        :class="{ 'opacity-25': form.processing }"
