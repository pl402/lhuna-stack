    ...
    return {
      ...
      data: {
        ...
        fields: (entity.fields || []).map(f => {
          ...
          // Spread first so we never mutate the original prop object
          const cloned = { ...f };
          if (cloned.name === 'id') {
        ...
        // Clone to break shared reference with props.entities — otherwise any
        // .push() here would mutate the prop and trigger the deep watcher,
        // resetting the canvas and losing unsaved (in-memory) entities.
...
// isDirty     → canvas changes not yet saved
// isUnpublished → saved but not yet published (generate + migrate)
...
watch(
  () => nodes.value.map(n => JSON.stringify({ id: n.id, data: n.data })).join('||'),
...
// ── Guardar esquema (lightweight, solo JSON) ───────────────────────────────
...
// ── Publicar sistema (save → generate → migrate) ──────────────────────────
...
const publishSystem = async () => {
  ...
  // Reload page data after a short delay so migrationsList updates
...
const removeField = (index) => {
  ...
  if (f.name === 'id') return; // protect ID
...
const removeRelation = (index) => {
  ...
  // Find related edge and remove it
...
<template>
  <AppLayout title="Diseñador de Entidades" :mainScrollable="false">
    <div class="w-full flex-1 flex min-h-0 relative bg-dark-base overflow-hidden transition-colors duration-300">
      ...
      <div class="flex-1 h-full min-h-0 relative select-none">
        ...
        <div class="absolute top-4 left-4 z-10 flex gap-2">
          ...
          <button 
            ...
            :disabled="saving"
            class="relative inline-flex items-center justify-center px-4 py-2.5 bg-dark-elevated hover:bg-dark-surface text-slate-200 hover:text-slate-100 rounded-xl font-bold transition duration-200 text-xs gap-2 disabled:opacity-50 border"
          ...
          <!-- Publicar: guarda + genera + migra en un solo flujo -->
          <button 
            type="button"
          ...
          >
            <!-- Unpublished indicator dot -->
            <span 
              v-if="(isDirty || isUnpublished) && !(showPublishModal && !publishDone && !publishError)" 
              class="absolute -top-1 -right-1 flex h-2.5 w-2.5"
            >
              ...
              <span class="relative inline-flex rounded-full h-2.5 w-2.5"
                :class="isDirty ? 'bg-amber-500' : 'bg-orange-500'"
              ></span>
            </span>
