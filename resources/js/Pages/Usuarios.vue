  ...
  <AppLayout title="Usuarios">
    <div class="py-4">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="
          ...
          ">
          <Buscador v-model="buscar" :orderByObject="orderByObject" ruta="usuarios" autofocus />
          <Tabla v-if="usuarios.data.length > 0">
            <template #col>
              <th class="px-4 py-1 w-20">
                <Ordena v-model="orderByObject" ruta="usuarios" :buscar="buscar" titulo="ID" campo="id" />
              </th>
              <th class="px-4 py-1">
                <Ordena v-model="orderByObject" ruta="usuarios" :buscar="buscar" titulo="Nombre" campo="name" />
              </th>
              <th class="px-4 py-1 w-72">
                <Ordena v-model="orderByObject" ruta="usuarios" :buscar="buscar" titulo="Email" campo="email" />
              </th>
            ...
            <template #row>
              <tr class="text-slate-700 hover:bg-slate-300 border-b border-slate-200" v-for="usuario in usuarios.data"
                :key="usuario.id">
                ...
                <td class="px-4 py-1">
                  <div v-for="p in usuario.roles" :key="p.id">
                    <span class="text-gray-200 mr-1 mb-1 px-1 rounded nowrap" :class="bg_color[p.name]">
                      {{ p.name }}
                    </span>
                  </div>
                </td>
                <td class="
                    ...
                    backdrop-blur
                  ">
                  <JetButton class="bg-blue-500 hover:bg-blue-700 text-white" @click="editaUsuario(usuario)">
            ...
            <template #pagination>
              <Paginador :links="usuarios.links" :total="usuarios.total" :to="usuarios.to" :from="usuarios.from" />
            </template>
          </Tabla>
          <div v-else class="text-center p-5 text-slate-500 text-md">
            ...
            <JetButton @click="nuevoUsuario" class="mt-2">
              <font-awesome-icon icon="add" class="mr-2" />Agregar Nuevo Usuario
            </JetButton>
          </div>
        </div>
      </div>
    ...
    <form class="flex flex-col" @submit.prevent="accion === 'new' ? nuevoUsuarioDo : editaUsuarioDo">
      <JetDialogModal v-if="estadoModalUsuario === true" :show="estadoModalUsuario" @close="estadoModalUsuario = false"
        ...
        <template #title>{{ accion === "new" ? "Crear nuevo usuario" : "Editar usuario" }}
          <JetDangerButton v-if="accion === 'edit'" @click="estadoModalEliminaUsuario = true" class="-mt-1 float-right"
            :disabled="
              ...
              (form.rol === 'Administrador' || form.rol === 'Contraloría')
            ">
        ...
        <template #footer>
          <div class="w-1/2">
            <JetButton @click="estadoModalUsuario = false" class="bg-opacity-95" type="cancel"
