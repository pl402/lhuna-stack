...
import { CheckIcon, SelectorIcon, PlusIcon, MinusIcon, XIcon } from "@heroicons/vue/solid";
...
let filteredOpcion = computed(() => {
  ...
  props.modelValue.forEach(element => {
    este_filtro = este_filtro.filter((opcion) => opcion.value !== element.value)
  });
  return este_filtro;
});
...
<template>
  <Combobox v-model="modelValue" :value="modelValue" ref="autocomplete" @update:modelValue="onChange" multiple>
    <div class="relative mt-1">
      ...
      <transition enter-active-class="transition ease-out duration-200" enter-from-class="transform opacity-0 scale-95"
        ...
        <ComboboxOptions class="
          ...
          <div v-if="filteredOpcion.length === 0"
            class="relative cursor-default select-none py-2 px-4 text-gray-400 text-xs bg-white/80 backdrop-blur-sm">
            Lista vacía
          </div>
  ...
  <transition enter-active-class="transition ease-out duration-200" enter-from-class="transform opacity-0 scale-95"
    ...
    <div v-if="modelValue.length > 0" class="block mt-2">
      <div v-for="opcion in modelValue" :key="opcion.id"
        class="bg-slate-600 inline-flex items-center text-sm mt-1 mr-1 rounded-full">
        <span class="ml-2 mr-1 leading-relaxed truncate max-w-xs bg-slate-600 text-white">{{ opcion.name }}</span>
        <button @click="modelValue.splice(modelValue.indexOf(opcion), 1)" class="w-6 h-6
          inline-block
          align-middle
          bg-slate-600
          hover:bg-slate-800
          ...
          hover:text-white
          rounded-full
          disabled:hover:bg-slate-600 disabled:hover:text-white/60" :disabled="props.disabled">
